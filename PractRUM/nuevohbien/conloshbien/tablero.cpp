...
void nuevaJugada(tTablero& tablero, tJugada jugada) {
	tablero.jugadas[tablero.contTablero] = new tJugada(jugada);
...
bool jugar(tTablero& tablero, tSoporte& soporte) {
	...
	while (pos != 0) {
		...
		if (pos > 0 && pos <= soporte.cont) {
			...
			//CAMBIADO TODO
			//jugada.fichas[jugada.numFichas]->color = soporte.ficha[pos - 1].color;
			//jugada.fichas[jugada.numFichas]->numFicha = soporte.ficha[pos - 1].numFicha;
			cont++;
			jugada.numFichas++;
		}
	}
	...
	if (cont == 1) {
		...
		cin >> pos;
		if (ponerFicha(*(tablero.jugadas[pos - 1]), jugada.fichas[0])) {//CAMBIADO TODO()*
			cout << "-> Colocada" << endl;
...
void delTablero(tTablero& tablero) {
	...
	for (int i = 0; i < tablero.contTablero; i++) {
		...
		tablero.jugadas[i] = nullptr;
	}
	tablero.contTablero = 0;
}